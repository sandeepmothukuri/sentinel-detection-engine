...
| Python tests | `pytest tests/` (207 tests) | Regressions: parameter parsing patterns, uniqueness, honest validation statuses, JSON artifacts parse, the declared toolchain matches what the gates import |
...
| Gallery drift | `scripts/generate_image_gallery.py --check` | `docs/images/gallery.md` is regenerated from `docs/evidence.md` and the capture manifest, so the page describing the evidence cannot drift from the register |
...
| `tests/test_evidence.py` | 21 | Evidence hygiene for images: no committed image carries EXIF or PNG metadata, every `Screenshot` entry records an environment, date and redaction statement, lab captures follow the naming convention, and an unfilled `Demonstrates` line fails the build |
...
| `tests/test_diagrams.py` | 15 | Everything drawn or registered about the content: numbers printed inside diagram sources, the architecture table-to-connector mapping, the workbook wireframe's panel parity, the absence of invented values in that wireframe, a register entry in `docs/evidence.md` for every committed image, the chart gates actually running in CI, and the social preview card agreeing with the rule files, plus the rule that no committed image is left without a page that displays it and that the gallery is generated rather than typed |
| `tests/test_playbooks.py` | 33 | The SOAR safety gates, checked against the deployed JSON rather than the README: no `MinimumSeverity` compared with `equals` (a floor that skipped everything above it), every destructive action behind a conditional, per-account exclusion and privileged-account checks, a device exclusion list consulted for both hostname fields, a firewall filter that rejects IPv6 and special-use ranges, and evidence that a VirusTotal response is actually read |
| `tests/test_workflows.py` | 8 | The workflow files GitHub will actually accept: no reserved secret name in `workflow_call`, every action pinned to a commit SHA with its release named, no `pull_request_target` or `workflow_run`, every `scripts/*.py` a workflow invokes exists, every gate the documentation claims CI runs is present, and actionlint reports nothing |
...
### What happens around the gate

![Release and pull-request automation](images/ci-cd/02-release-and-pr-automation.png)

*Generated by `scripts/render_diagrams.py` from
`docs/diagrams/ci-cd/02-release-and-pr-automation.mmd` — the workflows that run beside the validation
gate, and what each one is allowed to write.*
